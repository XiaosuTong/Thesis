...
In the second chapter, a new statistical method for the analysis of spatial 
seasonal-temporal dataset is proposed, named as Spatial Seasonal Trend Loess 
(SSTL). The chapter starts with the illustration of main steps of analysis 
routine of SSTL. Next, a spatial-temporal dataset from National Climatic
Data Center (NCDC) \cite{NCEI} is introduced and used as an example to explore
the routine step by step. The modeling results are evaluated by diagnostic 
visual display among the whole chapter.

In the third chapter, I illustrate a procedure to choose the best smoothing parameters
for spatial and temporal smoothing respectively. For the spatial smoothing, cross
validation method is used to choose the best smoothing span and degree in all
576 months collectively. The training dataset and testing dataset are decided
by using near exactly replicates framework to split the original dataset. For
the temporal STL fitting, an experiment of tunning smoothing parameters of STL 
is explored based on data after year 1950 to choose the best smoothing parameters 
in terms of prediction ability.

In the chapter two, we have demonstrated the SSTL routine in details, which 
includes four steps of analysis procedures. One of the advantages of SSTL routine
we haven't discussed yet is the flexibility of the SSTL for large and 
computationally heavy dataset. It comes free to embed the SSTL method into the 
divide and recombined framework. We can generate either by-month division or
by-station division of dataset to conduct the corresponding analysis on it. So
in the fourth chapter, we shift our gear to discuss the generalization of SSTL routine 
under the divide and recombined framework for big and complex spatial-temporal 
dataset. The SSTL routine now consists of a series of MapReduce jobs, and all 
fitting results are saved on HDFS as output. Meanwhile some of the diagnostics 
procedures discussed in chapter three are illustrated here with more details with 
respect to their parallel implementation. 

In the last chapter, we shift our gears to the analysis of the performance of the 
...
already demonstrated in the chapter three using 
cross-validation method. Another
