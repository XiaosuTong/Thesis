...
in the raw text file should be interpreted as 7.3 degrees. 
...
\end{description}
The total raw text files saved on HDFS are around 678MB. 

\subsubsection{Division by Month}

Next, a division by month of the whole data set is created based on plain text
files. Totally there are 836,875 lines in text files, and each line of the raw
text file corresponded to twelve monthly maximum temperature observations for a 
given station in a given year followed by their status code of observation 
(observed or missing) in terms of 0/1. In order to carry out the oncoming
analysis, we consider to transform each line of raw text files to be a more
analysis friendly structure which only has one observation per row
...
\begin{center}
  ...
  \captionof{figure}{Monthly number of observation}
  \label{obs.month}
\end{center}
\end{framed}

Here is how to refer a graph in this thesis. See Figure 
\href{../plots/obs_month.pdf}{\ref*{obs.month}}
