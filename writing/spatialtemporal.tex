...
Before we jump into the analysis of spatial-temporal dimension interactively, we
...
\begin{description}
  ...
  unique station ID and 1,236 months as the key, and a one-row R data.frame as the
  value. The value includes (1) station name, the name of station, (2) longitude, 
  degree of longitude of the station, (3) latitude, the latitude of the station, 
  (4) elevation, the meter of elevation the station, (5) year, year of the 
  observation, (6) month, month of year (January to December), (7) tmax, the 
  ...
  station ID and month index to station ID only. 
  \item[Reduce] 1,236 of one-row data.frame corresponding to one station are shuffled
  and aggregated to be one data.frame. Concretely all one-row data.frame who share
  the same station ID are shuffled and transfered to one reducer, and then R function 
  "rbind" is called to combine those one-row data.frame.
\end{description}

\subsection{Time Series Analysis of Each Subset}

After we created the database of division by station 
