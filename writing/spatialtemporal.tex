...
raw data. Next, we carry out the diagnostic for each components based on 
visualization.
...
conditional parameter into the local regression model.
...
result for this fit in the following diagnostic graph.
...
Next, we generate the diagnostic plots about the residual against to one of the 
...
problem in \texttt{stlplus} fit in the next section.
...
{\ref*{e4.abserrorlag.sw}}. Clearly, with smaller value of s.window there is 
difference of MAPE between the lag distance within a year and further lag distance.
Moreover, this difference is amplified when s.degree is equal to 2. For example, 
